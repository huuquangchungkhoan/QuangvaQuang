...
def fetch_latest_index_price(ticker: str) -> float:
    """Fetch real-time price for an index using the trading API."""
    try:
        url = "https://trading.vietcap.com.vn/api/market-data-service/v1/indexes/price"
        headers = {
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get("success") and data.get("data") and ticker in data["data"]:
                price = data["data"][ticker].get("price")
